...
class IndexRuleta extends Component {
	constructor(...props) {
	  super(...props);
	...
	showRuletaResult() {
	  ...
	  if (this.points_data === -1) {
		...
		swal(
		  'Ganó',
		  'Ha ganado ' + this.points_data + ' puntos',
		  'success'
		);
	  } else {
	...
	shopPremio(item) {
	  if (this.premios[item].precio <= this.state.total_points) {
		...
		swal(
		  'Comprado',
		  'Has comprado un item con éxito. (' +
			this.premios[item].premio +
			')',
		  'success'
		);
	  } else {
	...
	render() {
	  return (
		<div id="main" className={styles.fondo_rul}>
		  <div className="container">
			<div className="row">
			  <div className="col-md-4">
				<h2 align="center" className={styles.ruleta_puntos}>
				  TIENDA
				</h2>
				<br />
